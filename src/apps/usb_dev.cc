...
/**
 * @brief Set up the USB controller in device mode, clearing any previous
 * state.
 *
 */
 ...
 * device address in hardware is done in ep0_in_handler. This is because we
...
{
    // Set address is a bit of a strange case because we have to send a 0
    // length status packet first with address 0
...
void usb_handle_string_descriptor(volatile struct usb_setup_packet *pkt)
{
    ...
    else
    {
        ...
        len = usb_prepare_string_descriptor(
            dev_config.descriptor_strings[i - 1]);
    }

    usb_start_transfer(usb_get_endpoint_configuration(EP0_IN_ADDR),
                       &ep0_buf[0], MIN(len, pkt->wLength));
}
...
void usb_handle_config_descriptor(volatile struct usb_setup_packet *pkt)
{
    ...
    const struct usb_configuration_descriptor *d =
        dev_config.config_descriptor;
